# $Id: 04_out_fh.t,v 1.4 2005/10/02 22:35:36 tinita Exp $
use lib 'blib/lib';
...
sub test {
	...
	$htc->output($fh);
	close $fh;
	open my $f, '<', File::Spec->catfile('t', 'out_fh.htc') or die $!;
	open my $t, '<', File::Spec->catfile('t', 'out_fh.htc.output') or die $!;
	local $/;
	my $orig = <$f>;
	my $test = <$t>;
	for ($orig, $test) {
